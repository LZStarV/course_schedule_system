...
| 数据验证   | class-validator + class-transformer      | 最新版   | 运行时参数校验与转换                         |
| 身份认证   | JWT                                      | 9+       | 无状态身份认证、Token生成与验证              |
| HTTP客户端 | undici                                   | 5+       | 高效HTTP请求发送（内部服务调用）             |
| ORM框架    | @nestjs/sequelize + sequelize-typescript | 6+       | 数据库交互、模型定义、关联查询（装饰器）     |
...
- 日志工具：pino（本地开发使用 pretty 控制台输出）

- 接口文档：Swagger（NestJS集成，自动生成接口文档）

- 数据库迁移：后端统一迁移（Umzug + Sequelize），通过后端命令执行
...
统一入口 `POST ${backend.apiPrefix}/:method`（默认 `/api/:method`），请求/响应规范如下：

```ts
// 普通调用：Body 直接传 params（对象），响应采用 JSON‑RPC 结构
// 成功：{ jsonrpc:"2.0", id:null, result }
// 失败：{ jsonrpc:"2.0", id:null, error:{ code, message, data? } }

// 通知调用：Body 为 JSON‑RPC 封套且不带 id 时视为通知（不返回响应）
```

方法命名：`Auth.Login`、`Course.ListForStudent`、`Enrollment.Add`、`Admin.SetSelectTime` 等；方法名即路径段 `${apiPrefix}/Auth.Login`。
...
| 课程列表缓存 | `course:list:{academicYear}:{semester}:{status}:{page}:{page_size}` | 5分钟               | LRU（最近最少使用）            |
...
- 后端所有配置统一从 `packages/config` 获取（端口、基础地址、API 前缀、数据库连接参数）。
...
- 在 `main.ts` 中启用 CORS（`origin=devConfig.frontend.baseUrl`）。
- 若启动报 `EADDRINUSE: address already in use :::3001`，使用 `lsof -i :3001` 查找占用进程并结束，或临时调整端口。
...
- 数据校验：数据库操作前必须通过 class-validator + class-transformer 进行参数校验，防止无效数据入库

### 6.3 接口开发规范（RPC）

- 单入口：`POST /rpc`，请求体包含 `{ id, method, params }`

- 方法命名：领域 + 动作，形如 `Auth.Login`、`Course.ListForStudent`、`Enrollment.Add`

- 请求与响应：统一返回 `{ id, code, message, data }`；分页响应为 `PaginatedResponse<T>`（`data` + `pagination{ page, page_size, total, total_pages }`）

- 版本与兼容：方法层面保持后向兼容；如需大改，新增方法名而非改动旧方法语义

- 错误处理：业务异常映射为 `code>0` 与消息文本；系统异常由全局过滤器统一格式化

- 安全性：JWT 鉴权（`Authorization: Bearer`），方法级权限校验；重要操作记录审计日志
