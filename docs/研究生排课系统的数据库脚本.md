...
module.exports = {
  config: path.resolve('apps/backend/src/config', 'database.ts'),
...
const config: DatabaseConfig = {
  ...
  test: {
    ...
    database: process.env.DB_TEST_NAME || 'course_select_test',
...
export interface UserCreationAttributes extends Optional<
  UserAttributes,
  'id' | 'failed_login_attempts' | 'status' | 'created_at' | 'updated_at'
> {}
...
export default function UserProfileModel(sequelize: Sequelize) {
  class UserProfile
...
export interface DepartmentCreationAttributes extends Optional<
  ...
  'id' | 'level' | 'status' | 'display_order' | 'created_at' | 'updated_at'
> {}
  ...
  Department.init(
    {
      ...
      status: {
        type: DataTypes.ENUM(...Object.values(DepartmentStatus)),
        allowNull: false,
        ...
        validate: {
          ...
          max: function (value: number) {
            if (value > (this as any).capacity) {
              throw new Error('最小开班人数不能超过课程容量');
            }
          },
        },
        comment: '最小开班人数',
    ...
    },
    {
      ...
      hooks: {
        beforeUpdate: (course: Course) => {
          ...
          if (course.changed('status') && course.status === CourseStatus.PUBLISHED) {
...
export default function EnrollmentModel(sequelize: Sequelize) {
  class Enrollment
  ...
  Enrollment.init(
    {
      ...
      selection_method: {
        type: DataTypes.ENUM(...Object.values(SelectionMethod)),
        allowNull: false,
    ...
    },
    {
      ...
      hooks: {
        beforeSave: (enrollment: Enrollment) => {
          // 根据成绩计算是否通过
          if (enrollment.grade || enrollment.score !== undefined) {
...
module.exports = {
  up: async (queryInterface: QueryInterface) => {
    ...
    await queryInterface.addIndex('enrollments', ['student_id', 'course_id', 'status'], {
      name: 'enrollments_student_course_status_idx',
    });
...
module.exports = {
  up: async (queryInterface: QueryInterface) => {
    ...
    // 创建复合索引
    await queryInterface.addIndex('notifications', ['recipient_id', 'is_read', 'sent_at'], {
            ...
            roles: [
              {
                code: 'STUDENT',
                name: '学生',
                description: '学生角色，可以进行选课、查看成绩等操作',
                permissions: [
                  'course:browse',
                  'enrollment:select',
                  'enrollment:withdraw',
              ...
              },
              {
                code: 'TEACHER',
                name: '教师',
                description: '教师角色，可以创建课程、管理学生、录入成绩',
                permissions: [
                  'course:create',
                  'course:edit:own',
                  'course:delete:own',
              ...
              },
              {
                code: 'ADMIN',
                name: '管理员',
                description: '管理员角色，可以管理用户、审核课程、查看统计',
                permissions: [
                  'user:manage',
                  'course:audit',
                  'department:manage',
...
module.exports = {
  ...
  down: async (queryInterface: QueryInterface) => {
    await queryInterface.bulkDelete(
      ...
      {
        user_id: (
          await queryInterface.sequelize.query("SELECT id FROM users WHERE username = 'superadmin'")
        )[0][0]?.id,
      },
      {}
    );
...
module.exports = {
  up: async (queryInterface: QueryInterface) => {
    // 获取计算机学院和教师ID
    const [csDept] = (
      await queryInterface.sequelize.query("SELECT id FROM departments WHERE code = 'CS' LIMIT 1")
    )[0] as any[];

    const [teacher] = (
      await queryInterface.sequelize.query("SELECT id FROM users WHERE role = 'TEACHER' LIMIT 1")
    )[0] as any[];
    ...
    const courses = [
      {
        ...
        description: '本课程介绍计算机科学的基本概念、历史发展和主要领域。',
        objectives: '使学生了解计算机科学的基本概念和发展历程，为后续专业课程学习打下基础。',
        syllabus:
          '1. 计算机科学概述\n2. 计算机硬件基础\n3. 计算机软件基础\n4. 算法与数据结构简介\n5. 计算机网络简介\n6. 人工智能简介',
        assessment_method: '平时成绩30% + 期中考试30% + 期末考试40%',
        textbook_reference: '《计算机科学导论》（第3版），张三，清华大学出版社',
        attachments: [],
        restrictions: {
          grade_limits: [1, 2],
      ...
      },
      {
        ...
        description: '本课程介绍常用数据结构和基本算法设计分析方法。',
        objectives: '使学生掌握基本数据结构和算法的设计与分析方法，提高编程能力。',
        syllabus:
          '1. 算法分析基础\n2. 线性表\n3. 栈和队列\n4. 树和二叉树\n5. 图\n6. 排序算法\n7. 查找算法',
        assessment_method: '平时成绩20% + 实验30% + 期末考试50%',
        textbook_reference: '《数据结构（C语言版）》，严蔚敏，清华大学出版社',
        attachments: [],
        restrictions: {
          grade_limits: [2, 3],
          major_limits: ['计算机科学与技术', '软件工程'],
      ...
      },
      {
        ...
        location_details: {
          ...
          room: '301',
          capacity: 80,
          equipment: ['投影仪', '白板', '计算机', '数据库服务器'],
        },
        description: '本课程介绍数据库系统的基本原理、设计方法和应用技术。',
        objectives: '使学生掌握数据库系统的基本原理和设计方法，能够进行数据库应用系统开发。',
        syllabus:
          '1. 数据库系统概述\n2. 关系数据模型\n3. SQL语言\n4. 数据库设计\n5. 事务管理\n6. 数据库安全',
        assessment_method: '平时成绩30% + 课程设计30% + 期末考试40%',
        textbook_reference: '《数据库系统概念》（第7版），Abraham Silberschatz等，机械工业出版社',
        attachments: [],
        restrictions: {
          grade_limits: [3, 4],
          major_limits: ['计算机科学与技术', '软件工程'],
