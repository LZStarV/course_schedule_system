...
│   ├── backend/
│   └── frontend/
├── apps/backend/docker-compose.db.yml  # 仅数据库容器编排
├── packages/
├── docs/
...
```
apps/backend/src/modules/seed/services/
...
# 迁移状态
pnpm -F @apps/backend migrate:status

# 回滚（开发环境）
pnpm -F @apps/backend rollback:dev
...
# 批量假数据（院系/教师/学生/课程/排课/先修/选课）
pnpm -F @apps/backend seed:bulk

提示：批量假数据默认开启“已有数据则跳过”的守卫，若核心表已有数据将直接跳过注入并输出统计日志。
```
...
````bash
在 `apps/backend/src/db/migrations` 创建迁移文件（按时间戳命名），编辑后使用统一命令执行：
