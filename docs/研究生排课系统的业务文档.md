  ...
  children: [
    {
      ...
      accessible: (user, system) =>
        user.role === 'STUDENT' &&
        system.selection_status === 'IN_PROGRESS',
      visible: true,
      operations: {
        view: true,
        create: true,
      ...
      subOperations: [
        ...
        {
          code: 'withdraw_course',
          name: '退课',
          icon: 'remove',
          enabled: system.allow_withdraw,
        },
        {
          code: 'swap_course',
          name: '换课',
...
// 递归转换模块
const transformModuleToMenuOption = (
  ...
  return modules
    .filter(
      module =>
        module.visible &&
        module.accessible &&
        module.mobile_navigation !== false
    )
    .sort((a, b) => a.order - b.order)
    .slice(0, 5) // 最多显示5个
    .map(module => ({
...
async function checkRoutePermission(
  ...
  const modulePermission =
    permissionStore.getModulePermission(moduleCode);
...
const hasPermission = computed(() => {
  // 获取模块权限
  const modulePermission =
    permissionStore.getModulePermission(props.module);
  ...
  if (props.operation) {
    const hasOpPermission =
      modulePermission.operations[props.operation];

    // 如果需要检查资源权限
    if (
      props.resource &&
      typeof hasOpPermission === 'function'
    ) {
  ...
  'permission',
  {
    ...
    actions: {
      ...
      hasPermission(
        ...
        if (operation) {
          return (
            modulePerm.operations?.[operation] || false
          );
        }
    ...
    getters: {
      ...
      sidebarModules: state => {
        ...
        return modules
          .filter(
            module => module.visible && module.accessible
          )
          .sort((a, b) => a.order - b.order);
      },
...
export class PermissionObserver {
  ...
  private async checkRoutePermissions() {
    ...
    // 检查当前路由权限
    const hasPermission =
      await this.store.checkRoutePermission(route);
  ...
  getDataFilters(module: string): Record<string, any> {
    const modulePerm =
      this.store.getModulePermission(module);
...
class CourseAuditService {
  ...
  async getPendingCourses(
    ...
    // 3. 执行查询
    const courses =
      await CourseRepository.findPendingCourses(query);
  ...
  async auditCourse(
    ...
    // 1. 验证审核权限
    await this.validateAuditPermission(
      auditRequest.courseId
    );
  ...
  // 批量审核
  async batchAudit(
    batchRequest: BatchAuditRequest
...
#### 10.1.2 数据库容器配置（apps/backend/docker-compose.db.yml）
