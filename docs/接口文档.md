...
数据库通过 `apps/backend/docker-compose.db.yml` 管理：
...
- 统一入口：`POST ${backend.apiPrefix}/:method`（默认 `/api/:method`），方法名即路径段，如 `/api/Auth.Login`
- 请求体：普通对象作为 params；如需通知（不返回响应）可发送 JSON‑RPC 封套且不带 `id`
- 响应结构：遵循 JSON‑RPC 2.0
...
- `avoidConflict` 为占位参数，当前阶段未启用时间冲突过滤；后续将基于课程的周次与时段（class_schedules）实现冲突检查。
...
```json
{
  ...
  "params": {
    ...
    "endTime": "2025-09-07 20:00"
  }
}
```

#### 6.2.6 Course.ListByTeacher（新增）

- 方法名：`Course.ListByTeacher`
- 请求示例：

```json
{
  "id": "...",
  "method": "Course.ListByTeacher",
  "params": {
    "teacher_id": "uuid",
    "keyword": "数据库",
    "credit": 3,
...
```json
{
  ...
  "data": {
    "data": [
      /* 课程列表（与 Course.ListForStudent 一致） */
    ],
    "pagination": {
      "page": 1,
      "page_size": 10,
...
说明：管理员允许查看任意教师的课程；教师仅查看自身课程。

#### 6.2.7 Enrollment.ListMy（占位，后续由 class_schedules 支持）

- 方法名：`Enrollment.ListMy`
- 请求示例：

```json
{
  "id": "...",
  "method": "Enrollment.ListMy",
...
说明：课表数据源后续统一由 `class_schedules` 表提供；当前占位为分页结构，前端页面与类型已对齐。

#### 6.2.8 Admin.ListUsers（占位）
...
说明：删除为软删除，等价于将用户 `status` 更新为 `INACTIVE`；也可通过 `Admin.UpdateUser` 执行该操作。

#### 6.2.12 Auth.GetPermissions（返回结构示例）
...
2. **数据库**：PostgreSQL + Redis；仅数据库使用容器，迁移/种子由后端统一命令执行。
