...
export default {
  up: async (queryInterface: QueryInterface) => {
    await queryInterface.createTable('courses', {
      ...
      course_number: { type: DataTypes.STRING(20), allowNull: true },
      name: { type: DataTypes.STRING(200), allowNull: false },
      english_name: { type: DataTypes.STRING(200), allowNull: true },
      credit: { type: DataTypes.DECIMAL(3, 1), allowNull: false },
      credit_hours: { type: DataTypes.INTEGER, allowNull: false },
      course_type: {
        type: DataTypes.ENUM('COMPULSORY', 'ELECTIVE', 'GENERAL', 'EXPERIMENTAL', 'PRACTICE'),
        allowNull: false,
      },
      ...
      semester: { type: DataTypes.ENUM('FALL', 'SPRING', 'SUMMER', 'WINTER'), allowNull: false },
      capacity: { type: DataTypes.INTEGER, allowNull: false },
      min_quota: { type: DataTypes.INTEGER, allowNull: false, defaultValue: 1 },
      ...
      location_type: {
        type: DataTypes.ENUM('CLASSROOM', 'LAB', 'ONLINE', 'MIXED'),
        allowNull: false,
        defaultValue: 'CLASSROOM',
      },
      location_details: { type: DataTypes.JSONB, allowNull: true, defaultValue: {} },
      description: { type: DataTypes.TEXT, allowNull: true },
      objectives: { type: DataTypes.TEXT, allowNull: true },
      syllabus: { type: DataTypes.TEXT, allowNull: true },
      assessment_method: { type: DataTypes.TEXT, allowNull: true },
      textbook_reference: { type: DataTypes.TEXT, allowNull: true },
      ...
      favorite_count: { type: DataTypes.INTEGER, allowNull: false, defaultValue: 0 },
    ...
    await queryInterface.addIndex('courses', ['teacher_id'], { name: 'courses_teacher_id_idx' });
    await queryInterface.addIndex('courses', ['department_id'], {
      name: 'courses_department_id_idx',
    });
    await queryInterface.addIndex('courses', ['academic_year', 'semester'], {
      name: 'courses_academic_year_semester_idx',
    });
    await queryInterface.addIndex('courses', ['status', 'published_at'], {
